...
public class Last5sLookup {
    ...
    public Optional<MqttSendMessage> findLatest(String key) {
        ReadOnlyWindowStore<String, MqttSendMessage> store = streams.store(StoreQueryParameters.fromNameAndType(
                "last5s-store",
                QueryableStoreTypes.windowStore()));

        Instant now = Instant.now();
        try (WindowStoreIterator<MqttSendMessage> it = store.fetch(key, now.minusSeconds(5), now)) {
